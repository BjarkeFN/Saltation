...
struct Params {
	...
	uvec R;
	float waning_rate;
	map<int, mat> Immu; // Immunity to specific configurations of each epitope. 
	// The structure is as follows: Immu[n] is a N_epitopes x 2^(L_epitope) matrix, with each element a real number between 0 and 1.
	// This number signifies the degree of immunity to the given configuration at that epitope.
	// As such, they start out as 0.
	vector<int> infected; // ID of those individuals who are infected
    float xi; // Degree of strain-transcending immunity
...
bool check_if_fit(vector<int> genotype, int N_epitopes, int N_H, vector<int> configs, umat fitconfigs) {
	bool isfit = false;
	int conf;
	for (int i = 0; i < N_epitopes; i++) {
		conf = configs[i];
...
simdata run_outbreak_mutatingR(Params parameters) {
	...
	// Initiate (empty) Immu matrix for each agent:
	for (int i=0; i < parameters.I.size(); i++ ) {
		parameters.Immu[i] = mat(parameters.N_epitopes, pow(2, parameters.L_epitope), fill::zeros);
	}
	...
	vector<int> init_configs;
	init_gR = gen_mut(init_genotype, R_initial, 0.0, parameters.L_epitope, parameters.N_epitopes, parameters.N_H, parameters.d, parameters.dR_L_mean, parameters.fitconfigs, parameters.unfit_configs_fitness, parameters.fitconfigs_fitness);
	R_initgen = init_gR.R;
	init_configs = init_gR.configs;
	
	int retry_ctr = 0;
	bool isfit = false; // Check whether initial variant is one of the "fit configurations"
	
	isfit = check_if_fit(init_genotype, parameters.N_epitopes, parameters.N_H, init_configs, parameters.fitconfigs);
	
	while (R_initgen < parameters.R0_initial-0.1 or isfit) {
	...
	for (int n=0; n < I_initial; n++) {
		...
		for (int epitope=0; epitope < parameters.N_epitopes*parameters.N_H; epitope++) {
			// Add the immunity info to Immu:
            parameters.Immu[n](epitope,mutation.configs[epitope]) = 1.0;
		}
	...
	while ((generation < n_generations) and (N_previous > 0)) {
		...
		while (!infected_prev.empty()) {
			...
			int n = N_locs_prev[n_ID]; // Pick an infector from the previous generation
			infected_prev.pop_back(); // Remove said infector from the list of infected, to prevent double counting
			float r = draw_single_infectivity(R0s[0][n], alpha);
			poisson_distribution<> g(r);
			int R = g(gen); // Get reproductive number (integer)
			time_t tstart, tend;
			...
			for (m=0; m < R; m++) {
				...
				if (strainspecific) {
					if (parameters.S(infectee_ID) == 1) {
						...
						for (int epitope=0; epitope < parameters.N_epitopes; epitope++) {
							immunities.push_back(parameters.Immu[infectee_ID](epitope,mutation_tmp.configs[epitope]));
						}
						immunity_degree = *max_element(immunities.begin(), immunities.end());
					...
					if (parameters.S(infectee_ID) == 1) {
						...
						immunity_degree = parameters.Immu[infectee_ID].max(); // Immunity from any strain counts
						if (randu() < 1-immunity_degree) {
							infectable = true;
						}
					}
				...
				if (infectable) {
					...
					if (k_mut < 1) { // Just a way to discern high and low heterogenity in this simplified model
						float rnum = randu();
						if (rnum < 0.0001) { // Frequency of saltations
							// Jump size:
					...
					// Write immunity info about new infection to Immu:
					for (int epitope=0; epitope < parameters.N_epitopes; epitope++) {
						// Strain-transcending part:
						for (int conf=0; conf < pow(2,parameters.L_epitope); conf++ ) {
							parameters.Immu[infectee_ID](epitope,conf) = parameters.xi;
						}
						// Strain-specific part:
						parameters.Immu[infectee_ID](epitope,mutation_tmp.configs[epitope]) = 1.0;
					}
					N_loc++;
				}
			}
		...
		int I_loc = accu(parameters.I);
		for (int i=0; i < parameters.I.size(); i++ ) {
			// Update S and R on the basis of
			// immunity having waned:
			if (parameters.R(i)==1) {
				// Decrease immunity due to waning
				parameters.Immu[i] = parameters.Immu[i] - parameters.waning_rate*parameters.Immu[i];
			}
...
int main(int argc, char **argv) {
	...
	srand(seed);
	Params parameters;
    parameters.xi = 1.0; // Strain-transcending/cross immunity. 1.0 = fully transcending, 0.0=fully strain-specific
	parameters.generations = 300;
	parameters.I_initial = 2000;
	parameters.initially_susceptible_per_population = 2000000; // Lower to 200,000 in case of need for running on a low-memory machine for testing
    int initially_susceptible_per_population = parameters.initially_susceptible_per_population;
	parameters.R0_initial = 1.2;
	int Npops = 1;
	...
	parameters.mut_rate_high = 100; // Unless set from command line
	parameters.d = 3;
	parameters.L_epitope = 5;
	parameters.N_epitopes = 5;
	parameters.N_H = 1;
	parameters.dR_H_mean = 1.0;
	parameters.dR_L_mean = -100.0;

	parameters.waning_rate = 1/25.0;
	...
	if (argc > 1) {
		...
		cout << "k (default value) = " << parameters.alpha << endl;
	}
	...
	if (argc > 2) {
		...
		cout << "k_mut (default value) = " << parameters.k_mut << endl;
	}
	...
	if (argc > 3) {
		...
		cout << "dR_L_mean (default value) = " << parameters.dR_L_mean << endl;
	}
	...
	if (argc > 4) {
		...
		cout << "Saltation size (default value) = " << parameters.mut_rate_high << endl;
	}
